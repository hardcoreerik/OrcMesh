...
class TestByteLimit:
    ...
    def test_error_message_quotes_the_shared_constant(self):
        w = _connected_worker()
        errors = _errors_from(w)
        w.send_channel_text("a" * (MAX_MESSAGE_BYTES + 5), 0)
...
class TestNormalisation:
    ...
    def test_lone_cr_is_normalised(self):
        w = _connected_worker()
        w.send_channel_text("line1\rline2", 0)
        assert w._interface.sent[0]["text"] == "line1\nline2"

    def test_surrounding_whitespace_is_stripped(self):
        w = _connected_worker()
        w.send_channel_text("  hello  ", 0)
...
class TestRouting:
    ...
    def test_direct_send_targets_the_node(self):
        w = _connected_worker()
        w.send_direct_text("hi", 99887766)
        sent = w._interface.sent[0]
    ...
    def test_both_paths_request_ack(self):
        w = _connected_worker()
        w.send_channel_text("a", 0)
        w.send_direct_text("b", 1)
    ...
    def test_both_paths_enforce_the_same_limit(self):
        ...
        for send in (
            lambda w: w.send_channel_text(too_long, 0),
            lambda w: w.send_direct_text(too_long, 5),
        ):
            w = _connected_worker()
            send(w)
            assert w._interface.sent == []
